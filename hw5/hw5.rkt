  ...
  (cond [(var? e) 
        ...
        [(mlet? e)
          (let* ([v (eval-under-env (mlet-e e) env)]
                 [name (mlet-var e)]
                 [env (append env (list (cons name v)))])
          ...
          (let ([v1 (eval-under-env (call-funexp e) env)]
            ...
            (if (closure? v1)
              (let ([name (fun-nameopt (closure-fun v1))]
                    ...
                    [param (fun-formal (closure-fun v1))]
                    [env (closure-env v1)])
                (if (equal? name #f)
                  (eval-under-env body (append env (list (cons param v2))))
                  (eval-under-env body (append env (list (cons name v1) (cons param v2))))))
    ...
    (fun #f "ls"
      (ifaunit (var "ls")
        ...
        (apair (call (var "f") (fst (var "ls")))
               (call (call (var "mupl-map") (var "f")) (snd (var "ls"))))))))
